-- MEGA FIX V3: Final robust fix for Maintenance System
-- Corrects ENUM case sensitivity issues and ensures all relations
...
-- Updated Admin Policy with CASE-INSENSITIVE check to avoid enum errors
...
CREATE POLICY "Admins can view all maintenance requests"
    ...
    USING (
        EXISTS (
            ...
            AND role::text ILIKE 'admin%' -- This matches 'admin', 'Admin', 'super_admin' safely
        )
        OR 
        EXISTS (
            SELECT 1 FROM public.profiles
            WHERE id = auth.uid() AND (status = 'admin' OR status ILIKE 'admin%')
        )
    );
