...
export default function DriverMessaging() {
    ...
    const contacts = [
        { id: 1, name: 'مؤسسة النقل الأول (شاحن)', lastMessage: 'هل تحتاج مساعدة للوصول للموقع؟', time: '10:30 ص', unread: 1, online: true, phone: '0501234567' },
        { id: 2, name: 'محمد علي (شاحن)', lastMessage: 'تم التوصيل بنجاح', time: 'أمس', unread: 0, online: false, phone: '0559876543' }
    ];

    const messages = [
        { id: 1, text: 'السلام عليكم، أنا في طريقي لموقع التحميل.', sender: 'me', time: '10:15 ص' },
        { id: 2, text: 'وعليكم السلام، ممتاز. هل تحتاج مساعدة للوصول للموقع؟', sender: 'other', time: '10:30 ص' }
    ];
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {contacts.map(contact => (
                                ...
                                >
                                    ...
                                    {contact.unread > 0 && (
                                        <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-black">{contact.unread}</div>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="w-2/3 flex flex-col bg-white">
                        {activeChat ? (
                            <>
                                    ...
                                    {messages.map(msg => (
                                        <div key={msg.id} className={`flex max-w-[70%] ${msg.sender === 'me' ? 'mr-auto' : 'ml-auto'}`}>
                                            <div className={`p-4 rounded-2xl shadow-sm ${msg.sender === 'me'
                                                ...
                                                <p className={`text-[10px] mt-2 font-bold ${msg.sender === 'me' ? 'text-blue-100' : 'text-slate-400'}`}>{msg.time}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="p-4 bg-white border-t border-slate-100">
                                    <form onSubmit={handleSendMessage} className="flex gap-3">
                                        ...
                                        />
                                        <Button type="submit" className="h-14 w-14 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-600/20 shrink-0">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 space-y-4">
                                ...
                                <h3 className="font-black text-xl text-slate-500">اختر محادثة</h3>
                            </div>
                        )}
                    </div>
                </Card>
