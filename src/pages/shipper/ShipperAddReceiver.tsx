...
import { Users, Plus, Save, Phone, MapPin, User, Trash2, Edit2, Loader2 } from 'lucide-react';
...
export default function ShipperAddReceiver() {
    ...
    const fetchReceivers = async () => {
        if (userProfile?.id) {
            setLoading(true);
            try {
                const data = await api.getReceivers(userProfile.id);
                setReceivers(data || []);
            } catch (err) {
                toast.error('فشل جلب المستلمين');
            } finally {
                setLoading(false);
            }
        }
    ...
    const handleSave = async () => {
        ...
        try {
            await api.addReceiver(form, userProfile.id);
