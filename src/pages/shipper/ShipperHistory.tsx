...
export default function ShipperHistory() {
  ...
  const submitRating = async () => {
    ...
    if (!selectedLoad?.driver?.id) return toast.error('بيانات السائق غير متوفرة');

    setLoading(true);
    try {
      await api.submitRating({
        load_id: selectedLoad.id,
        driver_id: selectedLoad.driver.id,
              ...
              <table className="w-full text-right border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    ...
                    <th className="p-6 font-black text-slate-600 text-sm">التفاصيل</th>
                    <th className="p-6 font-black text-slate-600 text-sm">إجراءات</th>
                  </tr>
                </thead>
                    ...
                    <tr key={load.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-6">
                        <div className="flex items-center gap-2">
                          <span className="text-xl">{load.status === 'completed' ? '✅' : load.status === 'cancelled' ? '❌' : '⏳'}</span>
                          <span className={cn(
                            "text-xs font-black px-3 py-1 rounded-full",
                            load.status === 'completed' ? "bg-emerald-50 text-emerald-600" :
                              load.status === 'cancelled' ? "bg-rose-50 text-rose-600" :
                      ...
                      <td className="p-6">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-slate-800">{load.origin}</span>
                          <ChevronLeft className="text-slate-300" size={14} />
                          <span className="font-bold text-slate-800">{load.destination}</span>
                        </div>
                      </td>
                      ...
                      <td className="p-6">
                        <Button
                          ...
                          className="rounded-xl font-bold text-xs h-9 gap-2 hover:bg-slate-100"
                        >
                          <FileSearch size={16} /> عرض
                        </Button>
                      </td>
                      <td className="p-6">
                        <Button
                          ...
                          className="rounded-xl font-black text-xs h-9 gap-2 bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all"
                        >
                          <RotateCw size={14} /> تكرار الطلب
                        </Button>
                      </td>
                    </tr>
                  ))}
        ...
        <Dialog open={showRating} onOpenChange={setShowRating}>
          <DialogContent className="max-w-md rounded-[2.5rem] p-8 text-center bg-white border-none shadow-2xl">
            ...
            <div className="text-right mb-6">
              <label className="text-xs font-bold text-slate-400 mb-2 block">ملاحظات إضافية (اختياري)</label>
              <textarea
                value={ratingComment}
                onChange={(e) => setRatingComment(e.target.value)}
                className="w-full rounded-2xl bg-slate-50 border-none p-4 text-sm font-bold focus:ring-2 focus:ring-amber-400 min-h-[100px]"
