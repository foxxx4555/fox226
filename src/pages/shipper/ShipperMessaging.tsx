...
export default function ShipperMessaging() {
    const { userProfile } = useAuth(); // الحصول على المستخدم الحالي
    const location = useLocation();
    const queryParams = new URLSearchParams(location.search);
    const driverIdFromUrl = queryParams.get('driverId');
    ...
    useEffect(() => {
        if (userProfile) {
            Promise.all([
                api.getConversations(userProfile.id),
                api.getChatContacts(userProfile.id, 'shipper')
            ]).then(([convData, activeData]) => {
                ...
                activeData?.forEach((contact: any) => {
                    if (!mergedContacts.find(c => c.id === contact.id)) {
                        mergedContacts.push({
                            ...
                            phone: contact.phone,
                            lastMessage: 'متاح للمحادثة (السائق الحالي)',
                            time: ''
                        });
                    }
                });
                ...
                if (driverIdFromUrl && !activeChat) {
                    const existingContact = initialContacts.find(c => c.id === driverIdFromUrl);
                    if (existingContact) {
                        setActiveChat(existingContact);
                    } else {
                        supabase.from('profiles').select('*').eq('id', driverIdFromUrl).maybeSingle().then(({ data: profile }) => {
                            if (profile) {
                                const newContact = {
                                    id: profile.id,
    ...
    useEffect(() => {
        ...
        // جلب الرسائل القديمة
        api.getMessages(userProfile.id, activeChat.id).then(setMessages);
        ...
        const channel = supabase
            ...
            .on('postgres_changes', {
                ...
                if ((newMsg.sender_id === activeChat.id && newMsg.receiver_id === userProfile.id) ||
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!messageText.trim() || !userProfile || !activeChat) return;

        try {
            await api.sendMessage(userProfile.id, activeChat.id, messageText);
                            ...
                            <>
                                ...
                                <div className="flex-1 p-8 overflow-y-auto space-y-6 bg-slate-50/50 flex flex-col">
                                    {messages.map((msg, index) => (
                                        <div key={index} className={`flex max-w-[75%] ${msg.sender_id === userProfile?.id ? 'mr-auto' : 'ml-auto'}`}>
                                            <div className={`p-4 rounded-2xl shadow-sm ${msg.sender_id === userProfile?.id
