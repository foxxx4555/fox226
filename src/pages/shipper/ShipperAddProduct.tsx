...
import { Package, Plus, Save, Box, Trash2, Edit2, Layers, Tag, Loader2 } from 'lucide-react';
...
export default function ShipperAddProduct() {
    ...
    const fetchProducts = async () => {
        if (userProfile?.id) {
            setLoading(true);
            try {
                const data = await api.getProducts(userProfile.id);
                setProducts(data || []);
            } catch (err) {
    ...
    const handleSave = async () => {
        ...
        try {
            await api.addProduct(form, userProfile.id);
