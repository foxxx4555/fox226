...
export default function UserSupport() {
    ...
    useEffect(() => {
        ...
        // تفعيل الاستماع اللحظي لتحديث التذاكر تلقائياً (مثلاً عند رد الإدارة)
        if (userProfile?.id) {
            const channel = supabase
                .channel(`user-tickets-${userProfile.id}`)
                .on('postgres_changes',
                    { event: '*', schema: 'public', table: 'support_tickets', filter: `user_id=eq.${userProfile.id}` },
                    (payload) => {
                        fetchTickets();
                        if (payload.eventType === 'UPDATE') {
                            ...
                            if (newStatus !== oldStatus) {
                                toast.info(`تم تغيير حالة تذكرتك رقم ${payload.new.id.substring(0, 8)} إلى: ${newStatus === 'in_progress' ? 'قيد المعالجة' : newStatus === 'resolved' ? 'تم الحل' : 'مغلقة'}`);
                            }
                        }
                    }
                )
                                ...
                                <CardContent className="p-6">
                                    ...
                                    <div className="mt-4 pt-4 border-t border-slate-50">
                                        <Button
                                            ...
                                            className="w-full justify-between font-black text-blue-600 hover:bg-blue-50 rounded-2xl h-12"
                                        >
                                            <span className="flex items-center gap-2">
                                                <MessageSquare size={18} />
                                                {expandedTicketId === ticket.id ? 'إخفاء المحادثة' : 'פתח المحادثة الفورية مع الإدارة'}
                                            </span>
                                            <ChevronRight size={18} className={`transition-transform ${expandedTicketId === ticket.id ? '-rotate-90' : 'rotate-180'}`} />
                                        </Button>
